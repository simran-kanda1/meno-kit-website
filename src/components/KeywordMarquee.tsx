...
export function KeywordMarquee() {
  ...
  return (
    <section
      ...
      className="overflow-hidden border-y border-primaryDark/10 bg-surface py-6 sm:py-10"
    >
      <div className="marquee-row relative flex">
        <div className="marquee-track flex w-max items-center gap-16 px-8 sm:gap-20">
          {items.map((word, index) => (
            <span
              key={`${word}-${index}`}
              className="flex shrink-0 items-center gap-10 whitespace-nowrap font-display text-2xl text-primaryDark sm:gap-20 sm:text-4xl lg:text-5xl"
            >
              {word}
